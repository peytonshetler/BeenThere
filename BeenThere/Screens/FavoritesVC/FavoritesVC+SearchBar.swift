...
extension FavoritesVC: UISearchResultsUpdating, UISearchBarDelegate {
    ...
    func filterPlacesByScope(scope: SearchBarScope, filter: String) {
        ...
        case .location:
            ...
            filteredPlaces = fetchedResultsController.fetchedObjects!.filter {
                Helpers.Regex.isMatch(pattern: filter, string: $0.location!.locality) ||
                Helpers.Regex.isMatch(pattern: filter, string: $0.location!.administrativeArea)
            }
