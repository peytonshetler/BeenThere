...
class DetailsVC: BTPrimaryViewController {
    ...
    let divider = UIView()
    var tagDataSource: UICollectionViewDiffableDataSource<TagSection, TagItem>!
...
class TagItem: Hashable, Equatable {
    
    var id: UUID
    var name: String
    
    init(managedTag: BTTag) {
        self.id = managedTag.id!
        self.name = managedTag.name
    }
